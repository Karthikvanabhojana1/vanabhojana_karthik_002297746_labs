...
public class AnalysisHelper {
    ...
    public void getMaxLikeCommentPost(){
        ...
        Comment commentwithmaxLike=null;
        for(Comment c:data.getComments().values()){
            if(commentwithmaxLike==null){
                commentwithmaxLike=c;
            }
            if(c.getLikes()>commentwithmaxLike.getLikes()){
                commentwithmaxLike=c;
            }
        }
        int postId=commentwithmaxLike.getPostId();
    ...
    public void getPostwithMostComments(){
        DataStore data=DataStore.getInstance();
        Post postwithmaxLike=null;
        for(Post p: data.getPosts().values()) {
              if(postwithmaxLike==null){
                postwithmaxLike=p;
            }
            if(p.getComments().size()>postwithmaxLike.getComments().size()){
                postwithmaxLike=p;
            }
        }
        int postId=postwithmaxLike.getPostId();
        System.out.println("Q3 - post with most comment: "+ postwithmaxLike.getPostId());
    ...
    public void getPassiveCommentUsers() {
        ...
        System.out.println("Q5 - The following users have the leasts comments: ");
    ...
    public void getPassiveAndActiveOverallUsers() {
        ...
        System.out.println("Q6 - The following users have overral been passive: ");
